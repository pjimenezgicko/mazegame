...
class MazeWorld extends World {
  ...
  public WorldScene makeScene() {
    ...
    for (Edge e : this.edges) {
      w.placeImageXY(new LineImage(new Posn(0, -node), Color.BLACK), e.to.x * node,
          e.to.y * node - node / 2); // top
      w.placeImageXY(new LineImage(new Posn(0, node), Color.BLACK), e.to.x * node,
          e.to.y * node + node / 2); // bottom
  ...
  void KruskalsAlg() {
    ...
    edges.remove(0);
    /*
     * HashMap<String, String> representatives; List<Edge> edgesInTree; List<Edge>
     * worklist = all edges in graph, sorted by edge weights;
     * 
     * initialize every node's representative to itself While(there's more than one
     * tree) Pick the next cheapest edge of the graph: suppose it connects X and Y.
     * If find(representatives, X) equals find(representatives, Y): discard this
     * edge // they're already connected Else: Record this edge in edgesInTree
...
class ExamplesMaze {
  ...
  void testRender(Tester t) {
    ...
    ex1.initEmptyMaze();
    ex1.bigBang(ex1.height * MazeWorld.NODE_SIZE, ex1.width * MazeWorld.NODE_SIZE, .5);
  }
}